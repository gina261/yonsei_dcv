...
import numpy as np
import common  # noqa: E401
import q1_warmups as warmups  # noqa: E401
import q1_tests as tests  # noqa: E401
...
def parse_args():
    ...
    if not (args.allwarmups or args.alltests):
...
def main():
    ...
    for fnName in toRun:
        ...
        for seed in SEEDS:
            ...
            if str(fnName) == 't3' and (data - data_copy).any():
                print("You should not change the input in %s on seed %s."
                      % (fnName, seed))
                success = False
                break
