...
from backprop import f1, f2, f3_y1, f3_y2
...
parser.add_argument(
    '--data-file',
    help='Path to a data file with input / output data',
    default='data/backprop.npy')
parser.add_argument(
    '--seed',
...
def main(args):
    ...
    if args.action == 'save-outputs':
        ...
        save_outputs(fns, args.data_file)
        return

    if args.action in ['check-outputs', 'check-both']:
        fns = ALL_FUNCTIONS if functions is None else functions
        check_outputs(fns, args.data_file)
...
def gradcheck(f, tolerance=1e-10, trials=10000):
    num_inputs = len(signature(f).parameters)
